...
var solar = (function() { //Javascript module pattern
    ...
    function populate() {
        ...
        removeFromMap(_rectangles);
        // Reset panels
        _rectangles = createModules();
        render(_rectangles);
    ...
    // Get center of the left most panel for a given roofWidth
    function getCoordinates(roofWidth, panelWidth, spacing, roofCenter) {
        var hypotenuse = roofWidth/2 - ( panelWidth/2 + spacing);

        var dy = Math.sin(20 * Math.PI / 180) * hypotenuse;
        var dx = Math.cos(20 * Math.PI / 180) * hypotenuse;

        //roofCenter  + dx
        var cx = new google.maps.geometry.spherical.computeOffset(roofCenter, dx, 270);
        //roofCenter + dx + dy
        var cxy = new google.maps.geometry.spherical.computeOffset(cx, dy, 0);
    ...
    function createModules() {
        ...
        var cols = Math.floor((roofPolygonOptions.width) / (moduleWidth + moduleSpacing));
        var rows = Math.floor((roofPolygonOptions.length - moduleSpacing) / (moduleLength + moduleSpacing));
        
        // Make sure the panels fit length
        if(rows == 0)
            return [];

        for(var i = 0; i < cols; i++) {
            var center = getCoordinates(
                roofPolygonOptions.width - i * 2 * (moduleWidth + moduleSpacing), 
                moduleWidth, 
                moduleSpacing, 
                roofPolygonOptions.center)
            
            modulePolygons.push(createModulePolygon(moduleWidth, moduleLength, center));
        }
        
        return modulePolygons;
    }

    function createModulePolygon(w, l, center){
        var modulePolygonOptions = {
            center: center,
            width: w,
            length: l,
            rotation: 20,
        ...
        return new Polygon(modulePolygonOptions);
    }
    ...
    function removeFromMap(polygons) {
        for(var i = 0; i < polygons.length; i++) {
            var polygon = polygons[i];
            polygon.removeFromMap();
        }
